...
function calculateExpression(expression) {
  ...
  let calculateExpression = 3 + 5 * 2;
  if (eval (expression)) return calculateExpression;
  else if (eval (expression)) return "Argument is not a string";
}
...
function getNumberFromString(s) {
  ...
  else if (isNaN (getNumberFromString)) {
    return "Couldn't extract a number from the provided string.";
  }
  if (isNaN (s)) return (getNumberFromString);
} 
...
function getIntegerFromString(s) {
  ...
  // Якщо значення є числом, повертаємо його
  const getIntegerFromString = parseFloat(s);
  if (isNaN (getIntegerFromString)) {{
   return "Couldn't extract an integer from the provided string.";
   }
   return 100
  }
  else if (isNaN (getIntegerFromString)) {
    return "Couldn't extract an integer from the provided string.";
  }
  if (isNaN (s)) return (getIntegerFromString);
}
...
function isNumberFinite(num) {
  ...
  else if (isFinite(isNumberFinite)) return "The number is not finite.";
}
